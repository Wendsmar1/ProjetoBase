...
from projetobase.main import check_paths, cleanup_plan, main, run, scan_devhub, weekly_report
...
def test_cleanup_plan_and_json(tmp_path: Path):
    ...
    (f_root / "y").mkdir(parents=True)
    (f_root / "y" / "old.log").write_text("log", encoding="utf-8")
...
def test_main_cleanup_command(tmp_path: Path, capsys):
    c_root = tmp_path / "C_DevHub"
    f_root = tmp_path / "F_DevHub"
    c_root.mkdir()
    f_root.mkdir()
    rc = main(["cleanup-plan", "--c-root", str(c_root), "--f-root", str(f_root), "--out-dir", str(tmp_path)])
    assert rc == 0
    assert "Relatorio gerado:" in capsys.readouterr().out
